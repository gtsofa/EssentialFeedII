...
final class CacheFeedUseCaseTests: XCTestCase {
    ...
    func test_save_requestsNewCacheInsertionWithTimestampOnSuccessfulDeletion() {
        ...
        let items = [uniqueItem(), uniqueItem()]
        let localItems = items.map { LocalFeedItem(id: $0.id, description: $0.description, location: $0.location, url: $0.url)}
        sut.save(items) { _ in }
    ...
    private func expect(_ sut: LocalFeedLoader, toCompleteWith expectedError: NSError?, when action: () -> Void, file: StaticString = #filePath, line: UInt = #line ) {
        ...
        // store actions being replaced with the action closure
        action()
